...
const DEPT_ICON_PLACEMENTS: IconPlacement[] = (() => {
	...
	for (let i = 0; i < ICON_COUNT; i++) {
		const phi = phiStart + ((phiEnd - phiStart) * i) / Math.max(1, ICON_COUNT - 1);
		out.push({
			tx: -Math.cos(phi) * r,
			ty: -Math.sin(phi) * r,
			delay: i * 0.07,
...
export default function JoinUsPage() {
	...
	const handleCardInteraction = useCallback((e: React.MouseEvent, index: number) => {
			...
			<main id="joinus-main" className="joinus-main">
				<section className="joinus-hero" aria-labelledby="joinus-hero-title">
					<div>
						...
						<h1 id="joinus-hero-title" className="joinus-hero__title">
							...
							<span className="joinus-hero__title-sub">期待你的选择，一起把想法做成作品！</span>
						</h1>
						<p className="joinus-hero__lead">欢迎有热情、愿意长期投入的同学，希望共同经营一个大家庭！</p>
					...
					<img className="joinus-hero__img" src={assetUrl(heroImage)} alt="" aria-hidden="true" />
				</section>
				<section id="dept-title" className="joinus-section" aria-labelledby="dept-title-heading">
					<div className="joinus-section__head">
						...
						<p className="joinus-section__desc">当前开放 VUP、视频、美术、文案、技术与直播六个方向，你可结合兴趣与作品基础选择意向组别！</p>
					</div>
					<div className="joinus-grid joinus-grid--dept" ref={gridRef}>
						{DEPTS.map((d, idx) => {
							...
							return (
								<article
									key={d.title}
									className={`joinus-card${isActive ? ' joinus-card--active' : ''}`}
									onClick={(e) => handleCardInteraction(e, idx)}
									onMouseEnter={() => {
										if (isTouchDevice.current) return;
										setActiveCard(idx);
									}}
								...
								>
									...
									<h3>{d.title}</h3>
									<p>{d.desc}</p>
									<div className="joinus-card__icons" aria-hidden="true">
										{icons.map((icon, i) => (
											<img key={i} className="joinus-card__icon" src={icon.src} alt={icon.alt} style={iconStyle(placements[i]!)} />
										))}
									</div>
									<div className="joinus-card__corner-blur" aria-hidden="true" />
								</article>
							);
						})}
					</div>
				...
				<section id="process-title" className="joinus-section" aria-labelledby="process-title-heading">
					<div className="joinus-section__head">
						...
						<p className="joinus-section__desc">先定意向，再提交报名！</p>
					</div>
