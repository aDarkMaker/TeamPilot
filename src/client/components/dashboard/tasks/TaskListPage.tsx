...
export default function TaskListPage() {
	const { toast, show: showToast } = useDashboardToast();
	...
	const load = useCallback(async () => {
		...
		try {
			...
			showToast({ text: e instanceof Error ? e.message : '加载任务失败', type: 'err' });
		} finally {
			setLoading(false);
		}
	}, [showToast]);
	...
	const onDecide = async (task: TaskCard, status: 'accepted' | 'leave') => {
		...
		if (isStarted(task)) {
			showToast({ text: '现在想改，晚了～', type: 'info' });
			return;
		}
		setBusyId(task.id);
		try {
			...
			showToast({ text: status === 'accepted' ? '我为你的勤劳而喜悦' : '我为你的懒惰不喜悦', type: 'ok', durationMs: 1800 });
		} catch (e) {
			showToast({ text: e instanceof Error ? e.message : '更新状态失败', type: 'err' });
		} finally {
			setBusyId(null);
		}
	};
