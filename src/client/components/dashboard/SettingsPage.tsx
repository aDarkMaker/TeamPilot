...
function SettingsInput(props: InputHTMLAttributes<HTMLInputElement> & { className?: string }) {
	return <input {...props} className={`settings-input ${props.className ?? ''}`} />;
}

function SettingsTextarea(props: TextareaHTMLAttributes<HTMLTextAreaElement> & { className?: string }) {
	return <textarea {...props} className={`settings-textarea ${props.className ?? ''}`} />;
}

function SettingsButton(props: ButtonHTMLAttributes<HTMLButtonElement> & { variant?: 'primary' | 'secondary' }) {
	const { variant = 'primary', className, ...rest } = props;
	return (
		<button {...rest} type={rest.type ?? 'button'} className={`settings-btn ${variant === 'secondary' ? 'secondary' : ''} ${className ?? ''}`}>
			{props.children}
		</button>
	);
}

function SettingsFileInput(props: { accept: string; disabled?: boolean; label: string; onPick: (file: File | null) => void }) {
...
export default function SettingsPage() {
	...
	useEffect(() => {
		if (loading) return;
		if (
			originalRef.current.nickname === '' &&
			originalRef.current.signature === '' &&
			originalRef.current.qq === '' &&
			(nickname || signature || qq)
		) {
	...
	return (
		<div className="settings-page">
				...
				<section className="settings-section">
					...
					<div className="settings-avatar-row" style={{ marginBottom: '0.95rem' }}>
						...
						<div className="settings-actions-inline">
							<SettingsFileInput accept="image/jpeg,image/png,image/webp" disabled={saving} label="上传" onPick={(f) => void uploadAvatar(f)} />
					...
					<div className="settings-bg-preview">
						<img className="settings-bg-img" src={bgUrl ?? assetUrl(bgDefault)} alt="" loading="lazy" />
						<div className="settings-bg-overlay">
							<input
								...
								className="settings-file-input"
							/>
							<label htmlFor={bgPickId} className={`settings-bg-overlay-btn ${saving ? 'disabled' : ''}`}>
								修改
							</label>
							<button type="button" className="settings-bg-overlay-btn secondary" disabled={saving} onClick={() => void resetBg()}>
								恢复默认
							</button>
						</div>
					</div>
					...
					<section className="settings-section">
						...
						{biliBound && biliAvatar ? (
							...
							<div className="settings-bili-qrcode-wrap">
								...
								<SettingsButton variant="secondary" onClick={cancelBiliBind}>
									取消
								</SettingsButton>
							</div>
						) : (
							<SettingsButton variant="secondary" onClick={() => void startBiliBind()}>
								绑定B站账号
							</SettingsButton>
						)}
					</section>
			...
			{birthdayPicker.open ? (
				<div className="time-picker open" role="dialog" aria-modal="true" onClick={() => setBirthdayPicker({ open: false, field: null })}>
