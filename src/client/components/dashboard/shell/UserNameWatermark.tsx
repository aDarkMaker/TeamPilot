...
import { getShanghaiYmd } from '@/lib/timeCst';
...
export default function UserNameWatermark() {
	...
	const [today, setToday] = useState(getShanghaiYmd);
	...
	useEffect(() => {
		...
		const timer = window.setInterval(() => setToday(getShanghaiYmd()), 60_000);
	...
	return <div className="dash-watermark" aria-hidden="true" style={{ backgroundImage: buildTile(label), backgroundSize: '17.5rem 10.625rem' }} />;
}
